...
if ($mform->is_cancelled()) {
   ...
   $existingdata = $DB->get_record('local_plugin', array('username' => $username));
   if ($existingdata) {
    echo "<script>
  ...
  if($employee_number == '' || $organizational_unit == '' || $position == ''){
    ...
    $insertdata->username = $text->username;
    $insertdata->firstname = $text->firstname;
    $insertdata->lastname = $text->lastname;
    $insertdata->email = $text->email;
    $insertdata->employee_number = $text->employee_number;
    $insertdata->organizational_unit = $text->organizational_unit;
    $insertdata->position = $text->position;
    ...
    $insertdata->organizational_unit = $text->organizational_unit;
    $insertdata->position = $text->position;
    $existingdata = $DB->get_record('local_plugin', array('username' => $text->username));
    $success = $DB->insert_record('local_plugin', $insertdata);
  }
}
}
